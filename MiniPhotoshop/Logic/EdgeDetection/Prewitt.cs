...
namespace MiniPhotoshop.Logic.EdgeDetection
{
    public class Prewitt
    {
        ...
        {
            // 1. Setup LockBits (Akses Memori Cepat)
            ...
            byte[] resultBuffer = new byte[bytes]; // Buffer untuk hasil

            Marshal.Copy(srcData.Scan0, pixelBuffer, 0, bytes);
            int stride = srcData.Stride;

            // 2. Definisi Kernel Prewitt (3x3)
                ...
                for (int x = 1; x < width - 1; x++)
                {
                    ...
                    for (int ky = -1; ky <= 1; ky++)
                    {
                        for (int kx = -1; kx <= 1; kx++)
                        {
                            ...
                            int val = pixelBuffer[pos]; // Ambil Blue channel (diasumsikan grayscale)
